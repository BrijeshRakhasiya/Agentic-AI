...
def query_groq(prompt: str) -> str:
    ...
    data = {
        "model": GROQ_MODEL,
        "messages": [
            {"role": "system", "content": "You are an expert resume analyzer. Answer based only on the resume content provided."},
...
def run_resume_rag(pdf_path: str, query: str):
    ...
    context = "\n".join(top_chunks)
    prompt = f"""
You are analyzing a resume for insights.

Use only the following resume content to answer the question clearly and accurately.
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage:\n  python rag_resume_groq.py <resume.pdf> <question>")
        sys.exit(1)
